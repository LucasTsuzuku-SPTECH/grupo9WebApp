...
async function perguntar(req, res) {
    ...
    const data = req.body.dadosCombinados;

    try {
        const resultado = await gerarResposta(pergunta, data);
        res.json({ resultado });
    } catch (error) {
...
async function gerarResposta(mensagem, data) {
